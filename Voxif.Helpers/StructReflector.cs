...
namespace Voxif.Helpers.StructReflector {
    public static class StructReflector {
        private static ReflectedList Prepare(string file, string version, ReflectedList refListSource = null) {
            ReflectedList refListDest = refListSource ?? new ReflectedList();

            Assembly assembly = Assembly.GetExecutingAssembly();
            string structPath = assembly.GetName().Name + "." + typeof(StructReflector).Namespace + "." + file;
            using(StreamReader reader = new StreamReader(assembly.GetManifestResourceStream(structPath + version + ".txt"))) {

                string baseVersion = reader.ReadLine();
                if(!String.IsNullOrEmpty(baseVersion)) {
                    Prepare(file, baseVersion, refListDest);
                }

                Console.WriteLine("Prepare " + version);
        ...
        public static IStructReflector Load(string file, string version, int pointerSize) {
            ReflectedList reflectedList = Prepare(file, version);
